...
const EXEMPT: ReadonlyArray<{ file: string; size: number; why: string }> = [
    {
        ...
        size: 21,
        why: "`.diff-hunk-revert` draws the per-hunk revert control as a `»` character. An icon that happens to be text, sized to the action strip it stands in rather than to anything a reader reads. 21 is the ceiling for a 30px strip, not a taste call -- the font box is ascent+descent ≈ 1.29em, so the glyph must stay under 30/1.29 = 23.2px or it overflows `.diff-viewport` wherever a hunk sits flush against the top of the scroller.",
    },
    {
