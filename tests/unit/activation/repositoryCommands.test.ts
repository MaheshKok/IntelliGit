...
const mocks = vi.hoisted(() => {
    ...
    return {
        ...
        pushFileRepositoryFromContext: vi.fn(async () => undefined),
...
describe("registerRepositoryCommands", () => {
    ...
    it("registers Compare with Branch or Tag through the file-context wrapper", async () => {
        const gitOps = makeGitOps();
        registerRepositoryCommands(makeDeps(gitOps));
        const context = { clicked: "file" };
    ...
    it("registers scoped file fetch with commit-panel and active-graph refresh callbacks", async () => {
        const gitOps = makeGitOps();
        const refreshCommitPanels = vi.fn(async () => undefined);
        const deps = makeDeps(gitOps);
        deps.refreshService = vi.fn(
        ...
        await mocks.commands.get("intelligit.fileContext.fetch")?.(context);

        expect(mocks.fetchFileRepositoryFromContext).toHaveBeenCalledWith(
            context,
            gitOps,
            expect.any(Function),
    ...
    it("registers scoped file push with refresh callbacks and extension secrets", async () => {
        const gitOps = makeGitOps();
        const refreshCommitPanels = vi.fn(async () => undefined);
        const deps = makeDeps(gitOps);
        deps.refreshService = vi.fn(
        ...
        await mocks.commands.get("intelligit.fileContext.push")?.(context);
        ...
        const refresh = mocks.pushFileRepositoryFromContext.mock.calls[0]?.[2];
        await refresh?.();
        expect(refreshCommitPanels).toHaveBeenCalledTimes(1);
    });
