 ...
 * the wrong type, or failed to compile. `ciCdFilterInvalid` is true only when a non-empty
 * string was supplied but did not compile, so the caller can surface a warning.
 */
 ...
 * non-empty string `ciCdFilter` is compiled case-insensitively; a compile failure leaves
 * `ciCdPattern` unset and flags `ciCdFilterInvalid`. The returned object and its provider
 * map are frozen.
 *
 ...
 * flag; a syntax error yields no pattern and sets the invalid flag.
 *
