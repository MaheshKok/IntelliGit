...
- Added `intelligit.commitChecks.enabled` (turn the whole badge feature off), `intelligit.commitChecks.providers` (per-provider `github`/`gitlab`/`bitbucket-cloud`/`bitbucket-server` toggles), and `intelligit.commitChecks.ciCdFilter` (override which check names count as CI/CD) settings. All are read once at activation and take effect after a window reload; an invalid `ciCdFilter` regex falls back to the built-in pattern and shows a one-time warning. A disabled provider yields no badge for its remote and does not fall through to a secondary remote's enabled provider, and a disabled feature renders no badge button at all (no network calls).
