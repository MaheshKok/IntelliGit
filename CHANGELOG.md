...
- Changes are now marked in a stripe beside the scrollbar, so a reader who has not scrolled yet can see where the differences are and whether any lie below the fold — the affordance PyCharm offers in the same place. Each mark takes the colour of the block it points at, and clicking one jumps to it. The stripe is a redundant view of content the scrollbar already reaches from the keyboard, so it is hidden from screen readers and adds no tab stops, which is the call VS Code's own minimap makes; a file shorter than the viewport marks only the part of the lane its content actually occupies, rather than pointing confidently at blank space.
