...
- The working-tree side of a diff is now editable in place. It is a real text surface bound to VS Code's own document rather than a copy of it, so there is no apply step and no separate dirty state: the editor's undo stack, its dirty marker and its save prompts all behave exactly as they do in an ordinary tab, and an edit made in the diff *is* an edit to the file. The pane renders through the same shared code block as the immutable side and the three-pane merge editor, so it keeps line numbers, syntax highlighting, word highlighting and row alignment with the opposite pane instead of degrading into a plain text box. Editing follows the merge editor's gesture: a double-click opens one block, Escape cancels it, and leaving it commits the block as a single change rather than one per keystroke. Only a side that is genuinely a working tree becomes editable — a comparison between two immutable revisions stays read-only on both sides, and there are no accept, discard, revert or stage controls; this is editing, not a second staging surface.
- Editing works from the keyboard and with input methods that compose. A block can be reached with Tab and opened with Enter or F2, and it carries an accessible name, so a screen reader announces it instead of meeting an unnamed region. A composition in progress — how Chinese, Japanese and Korean input works, among others — is no longer torn out from under the input method when the file changes underneath the diff: the change is recorded and applied once the composition ends, and a draft that outlived such a change can still be seen but can never be written back against text it was not measured against. A large paste is scrollable immediately rather than after it is committed.
- The local side of a stash or shelf diff is editable too, and now shows the live file. Those two comparisons previously diffed the working tree against a copy of itself, which made the local pane read-only by construction and left it showing the bytes captured when the diff opened — so a file changed elsewhere while the diff stayed open silently disagreed with the pane. Both now open the real workspace file. The stash side still resolves through the stash's own commit hash rather than its position in the list, so renumbering the stash cannot swap what the opposite pane shows.
- Find is back. 0.28.0 replaced the built-in diff editor everywhere and named find-in-editor among the affordances that cost; both diff webviews now enable VS Code's own find widget, so next and previous, match highlighting, and the regular-expression and whole-word toggles all behave as they do anywhere else in the editor. Go-to-definition, the minimap and the inline-diff toggle are still gone.
- Changes are now marked in a stripe beside the scrollbar, so a reader who has not scrolled yet can see where the differences are and whether any lie below the fold — the affordance PyCharm offers in the same place. Each mark takes the colour of the block it points at, and clicking one jumps to it; Alt+Down and Alt+Up make the same jump from the keyboard, stopping at the first and last change rather than wrapping round, and standing aside while an open edit block or an input method has the arrow keys. The stripe itself stays hidden from screen readers and adds no tab stops — one per change would be hundreds on a real diff — which is the call VS Code's own minimap makes, where the picture is decorative and the change navigation lives on a key. A file shorter than the viewport marks only the part of the lane its content actually occupies, rather than pointing confidently at blank space.
...
- Added IntelliGit's own side-by-side diff viewer, and routed every read-only diff surface through it: changed files in the Commit panel, a file picked out of a commit in the Graph view, in commit info or in the undocked window, a single file from a stash or from a shelf entry, and comparing the open editor against a branch or a revision. Every one of those previously handed off to VS Code's built-in diff editor, so the extension showed merge conflicts in one visual language — the PyCharm-style panes the merge editor has always used — and every other comparison in another. The renderer behind the merge editor has been extracted into a shared core and the viewer built on top of it, so both surfaces now draw the same panes, the same centre line-number gutters, the same curved connectors between corresponding blocks, and the same Shiki syntax highlighting. The viewer keeps the merge editor's ignore-whitespace and word-highlight toggles, states explicitly when two sides differ only in their trailing newline rather than showing panes that look identical, and refreshes while it is open: saving the file, or moving `HEAD` or the branch a pane is labelled with, re-resolves only the sides that can actually have changed. A stash or shelf side is deliberately frozen when the diff opens, because the position that names it addresses a different object as soon as the list moves, and a refresh that re-ran it would silently swap the pane's subject.
- The size limits the viewer refuses to work above are measured rather than guessed, from a corpus spanning small, typical, large and pathological files: 210,094 bytes and 5,226 lines per side, nine million cells for the line-alignment matrix, and 439,048 bytes for the payload handed to the webview. All four are checked before any diff is computed, because the purpose of such a limit is to refuse the work rather than to survive it. The payload limit is the one no per-side check can stand in for — how large text becomes once serialized depends on what the text contains, and two sides that each pass the byte limit can still carry 420,188 bytes into a 439,048-byte budget with nothing left over.

### Changed

- The built-in diff editor is no longer reachable from any of the surfaces listed above, and there is no setting that brings it back. Showing one diff everywhere is the point of the change rather than a side effect of it, but the cost is real and worth stating plainly: find-in-editor, go-to-definition, the minimap and the inline-diff toggle are affordances of the built-in editor, and the viewer has none of them today. The built-in editor is still opened automatically, exactly as before, for content the viewer will not render — a binary file, one that is not valid UTF-8, a symbolic link, a submodule, a pair over the limits above, or a path present on neither side — so those cases behave as they always did instead of showing empty panes. Whole-stash and whole-shelf overviews are unchanged and still open natively; only single-file comparisons move. Renames are not modelled: a renamed file still appears as a separate deletion and addition, as it did before.

### Fixed

- Fixed the extension re-reading Git on every keystroke. The auto-refresh service subscribed to VS Code's text-document change event without asking whether the document was actually dirty, so typing a single character into any open file scheduled a refresh of the commit panels, the branch list, the worktree list and the merge-conflict badge. None of those can change until a write lands, so each of those refreshes recomputed the answer it already had. An edit still sitting in an unsaved buffer is now ignored, and the refresh runs on the write that follows it.
- Fixed syntax highlighting occasionally colouring part of a line wrongly in the merge editor, and then keeping it wrong for the rest of the session. Shiki's first tokenization against a freshly built grammar can classify identical input differently from every later one — measured over eight identical mounts, `const hd0 = 0;` came back three times with the semicolon swallowed into the number literal and painted as part of it, and five times correctly. Tokenized lines are cached per line, so one cold result stayed wrong until the webview was recreated, and which line happened to be tokenized first varied from run to run, so the damage landed somewhere different each time. A throwaway line is now tokenized once per language before any real one, spending the cold call where nobody sees it.
...
- Fixed applying a shelf entry failing on Windows for every user. Writing a file back into the working tree opened it with a flag combination — write-only plus truncate, with no create — that libuv turns into a Win32 `TRUNCATE_EXISTING` disposition, and Windows rejects it outright with `EINVAL: invalid argument`. Every one of the thirty-two `EINVAL` failures in the Windows run came from this one call, while the sibling write in the recovery path — same helper, same platform, but creating a file rather than opening an existing one — never failed. The payload is now written to a newly created temporary file and moved into place, so neither flag is involved any more.
- Fixed shelf and recovery writes failing on Windows for every user. Three places asked whether Node *defines* the `O_NOFOLLOW` open flag, which it does everywhere, rather than whether the platform *honours* it, which Windows does not, and `open` rejected the resulting flag word with `EINVAL`. The question is now asked in one place and answered per platform. That flag guards against a symbolic link being swapped in at the instant a file is opened, and returning zero for it on Windows widens that window rather than removing a check: each of those callers already confirms the target is an ordinary file both before and after writing, or creates it exclusively, which refuses to follow a link on its own.
- Closed a window in which applying a shelf entry could write through a symbolic link planted underneath it. The check that the destination is an ordinary file and the write that relies on that check are two separate operations, and a link substituted in between was followed — the repeat check, made after the write, could only report an escape that had already happened. On macOS and Linux an open flag closed that window; Windows does not honour that flag, so there it was never closed at all. The payload is now written to a newly created temporary file beside the destination and moved onto it, because a move replaces whatever sits at the path rather than writing through it, on every platform and without relying on a flag one of them ignores. The replaced file's permissions are carried across, so a file that was executable stays executable.
- Fixed recovery and rollback addressing files by a path Git cannot resolve on Windows. Repository-relative paths were normalised into the platform's own spelling, so on Windows they came back with backslashes and were then handed straight to Git — which addresses paths with forward slashes on every platform, so `blocked\one.txt` resolves to nothing and index lookups and base-blob reads found no entry rather than reporting a problem. The same value is what a rollback names as retained and what the recovery journal is keyed by, so it also has to be spelled the same way for a shelf exported on one platform to import on another. The check that a path does not escape the repository still runs first, against the platform's own spelling, so only the way the result is written down has changed.
- Fixed a Git command that produced more output than allowed being reported as a failure rather than as truncated output. Whether the command had been deliberately stopped was recorded from the result of the stop request, which says whether a signal was *sent* — and that is false once the process is already gone. A second oversized chunk arriving after the first stop had taken effect reset the record, and the command then reported the very termination it had itself asked for as an error. Windows produces exactly that sequence, because terminating a process there does not wait for it to end and the output pipe keeps draining behind it. What the record means is that the stop was requested, which is known without consulting the result at all.
- Fixed a repository lock held by another process being reported as a missing file. When a lock turns out to be busy, its record is moved back where it was found and the caller is told the lock is busy — but that move was allowed to fail loudly, replacing the verdict with whatever the filesystem raised, typically `ENOENT`. Callers recognise the busy error in order to wait and retry, so every one of those recovery paths was skipped and the operation surfaced “no such file” about a lock that was merely in use. The restore is now best-effort, matching the branch beside it, and a record left behind is reclaimed by the existing stale-lock takeover on the next attempt.
- The repository lock's heartbeat now reports a failure to publish its record instead of discarding it. The heartbeat runs on a timer, where a thrown error would take the extension host down, so failures were swallowed — but a heartbeat that stops publishing leaves its timestamp frozen, and a frozen timestamp is exactly what another process reads as an abandoned lock. It takes the lock over, two owners then write to one repository, and nothing anywhere said why. The cause is now logged once per distinct failure rather than at every tick.

### Changed

- The Windows leg of the compatibility check now runs to completion, which it had never done before. Reaching that took fourteen further changes, none of which alter what an installed extension does. The cache that VS Code is unpacked into was being created only as a side effect of a macOS-only step, so on Windows it did not exist and the run ended in setup before a single test executed. The packaged extension's command-line tool is a `.cmd` file, which Node has refused to start without a shell since a 2024 security fix, so the installed-package check produced `spawn EINVAL` for as long as the job has existed. Several tests asserted the wording, the path spelling, or the process-signal behaviour that only POSIX systems produce; two timeouts were sized for the fastest platform rather than the slowest; and the tests whose input Windows cannot represent at all — file permission bits, characters the filesystem rejects — are now skipped there by name rather than failing. Five test teardowns were also routed through the retrying delete this repository already uses for the race where Git keeps writing into the object store after its command has returned. The Windows run now reports the same 4,202 tests as macOS and Linux, forty-five of them skipped as inexpressible on that platform.

## [0.27.4] - 2026-08-23

### Fixed

- Fixed every Shelf operation failing on Windows. Saving a shelf writes each file to a temporary name, renames it into place, and then asks the operating system to flush the containing folder, which is what makes the rename durable if the machine loses power. Windows lets that folder be opened and then refuses to flush it, so the write raised `EPERM: operation not permitted` after the file had already been put in place: the operation reported failure, and saving, restoring and the recovery journal all failed with it. Windows offers no equivalent call — SQLite and LevelDB skip it there for the same reason — so the flush is now skipped on Windows alone and still performed on macOS and Linux, where it works and where leaving it out would risk losing a shelf on an unclean shutdown. This was found the first time the portability check got far enough to run the test suite on Windows: two hundred and six failures, every one of them this.
...
- Open Repository is now blue and pull is orange, in both toolbars and in the branch menu. The two toolbars paint the same action from different sources — the Commit panel resolves a theme token, the Graph view's title bar can only load a static SVG — so the pair only agrees when the token's hue and the file's hue are the same one. Swapping rather than recolouring keeps the rule that no two actions in a bar share a hue, which pull's blue would otherwise have collided with.
