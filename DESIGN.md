...
**The Host Wins Rule.** Every color used in the UI is written as `var(--vscode-<token>, <fallback>)`. Introducing a bare hex value into a component is prohibited. A `var()` with *no* fallback is the same failure wearing a better disguise: the shelf warning count and the shelf-health banner both referenced `--vscode-inputValidation-warningBackground` bare, and that token is optional, so a theme that omits it rendered the product's two loudest warnings as unstyled text. The sanctioned exceptions are the ten graph lane colors documented above, because VS Code exposes no graph-lane theming variable, and the Pull glyph's orange in the "color" icon style (`ICON_ACCENTS.nativeOrange`), because the sidebar Graph title's Pull is a static SVG no token reaches and the Changes tab bar's Pull and the branch menu's Update must match it (#218). There is no other exception: a file-type badge palette of ecosystem brand colors once sat in `tokens.ts`, unreferenced, with three entries below even the 3:1 non-text floor — it was deleted rather than documented, because sanctioning it would have written a contrast failure into the system PRODUCT.md promises never to ship.
...
**The One Loud Thing Rule.** At most one Action Indigo element per surface. If a panel has two primary buttons, one of them is secondary and hasn't been told yet. The commit action row is the one sanctioned exception: Commit and Push are both Action Indigo because they are the two halves of finishing a change, and greying Push made the panel's second-most-used control read as a cancel. Push's caret inherits the same fill, since a grey sliver against a blue button reads as a separate action rather than as Push's own menu. The exception is the row, not the surface — nothing else on the commit panel may take the fill.
...
- **Commit message box:** The primary writing surface starts at a 220px composer height, with a 180px minimum. A visible localized label names the textarea; Generate/Stop sits beside the label, outside the writing area. The action row wraps as needed for translated labels. Commit and Push are both primary, and so is the push-options caret; see the One Loud Thing Rule for why this row is the exception. `primary` declares no border, so each of the three reserves a transparent one, or the disabled state loses its edge along with its fill.
